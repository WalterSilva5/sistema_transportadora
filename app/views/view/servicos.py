...
def servicos_por_veiculo(request):
    ...
    servicos = list(Servico.objects.filter(veiculo_idveiculo=veiculo.id).order_by('data'))
...
def cadastrar_servico(request):
    ...
    servico = Servico(descricao=descricao, data=data, valor=valor, veiculo_idveiculo=veiculo_idveiculo)
    servico.save()
    return redirect('/')